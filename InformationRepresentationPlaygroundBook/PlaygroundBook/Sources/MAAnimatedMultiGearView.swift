    ...
    internal func rotate() {
        ...
        if !stopRotation && !isRotating {
            ...
            UIView.animate(withDuration: duration, delay: 0, options: .curveLinear, animations: { () -> Void in
                ...
                switch self.style {
                case .singleGear:
                    self.arrayViews[0].transform = self.arrayViews[0].transform.rotated(by: self.arrayOfRotationAngle[0] / 720 * CGFloat(Double.pi))
                case .keepGears:
                    for i in 0..<self.arrayViews.count {
                        let view = self.arrayViews[i]
                        view.transform = view.transform.rotated(by: self.arrayOfRotationAngle[i] / 720 * CGFloat(Double.pi) / self.divisionFactor)
                    }
                }
